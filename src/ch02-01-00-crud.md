...
For the contract we need the passed data. To get the data we have two possibilities. 

First you can pass all values as single parameters to the function. This approach can seem a bit confusing. Another method could be pass  all values as a JSON string to the contract. In this second approach you have a pass less parameters to the contract.
...
In general, you need to pass at least two parameters, a key and a value. The key represents an ID that uniquely identifies the asset. The second value is the value assigned to that asset. More technically speaking, the value is represented as raw binary data.

That`s why the associated value of an asset can be a simple string, an integer, and a boolean or a complex data structure.
